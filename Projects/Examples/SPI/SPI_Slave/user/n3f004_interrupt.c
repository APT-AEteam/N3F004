...
void SPI0IntHandler(void) 
{
	U8_T byRev;
    // ISR content ...
	if((SPI0->MISR&SPI_INT_RX_OV)==SPI_INT_RX_OV)					//Receive Overrun Interrupt
	{
		SPI0->ICR = SPI_INT_RX_OV;
	} 
	else if((SPI0->MISR&SPI_INT_RXTO)==SPI_INT_RXTO)				//Receive Timeout Interrupt
	{
		SPI0->ICR = SPI_INT_RXTO;
	}
	else if((SPI0->MISR&SPI_INT_RXFIFO)==SPI_INT_RXFIFO)				//Receive FIFO Interrupt,FIFO can be set 1/8,1/4,1/2 FIFO Interrupt
	{
		SPI0->ICR = SPI_INT_RXFIFO;
		byRev=SPI0->DR;
	}
	else if((SPI0->MISR&SPI_INT_TXFIFO)==SPI_INT_TXFIFO)				//Transmit FIFO Interrupt
	{
		SPI0->ICR = SPI_INT_TXFIFO;
	}
